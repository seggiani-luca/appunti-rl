...
Ad esempio, una porta NOR sarà implementata con determinati circuiti, ma tutto ciò che interessa a questo corso è come si comporta logicamente: $A \ \text{NOR} \ B  = y$ significa $ y = 1 \Leftrightarrow (A == B) = 0 $.
...
Questo rende il codice in assembler quasi mai portatile.
...
I calcolatori lavorano con i numeri, e un numero si \textbf{rappresenta} su una certa base $\beta$.
Nel caso dei calcolatori questa è la base 2.
...
$$
x =  b_{n-1} \cdot 2^{n-1} + b_{n-2} \cdot 2^{n-2} + ... + b_1 \cdot 2 + b_0 = \sum_{i=0}^{n-1} b_i \cdot 2^i
$$
...
Questo generalizza a qualsiasi base $\beta$ (per noi $\beta = 2$) come:
\begin{algorithm}
\caption{DIV-MOD}
\begin{algorithmic} % The number indicates where to start line numbering
    \STATE \textbf{Input:} $x$ in base arbitraria
    \STATE \textbf{Output:} $X$ rappresentazione in base $\beta$
    ...
    \WHILE{$q \neq 0$}
        \STATE $r \gets q \mod \beta$ % Calculate remainder
        \STATE Metti $r$ in $R[i]$ % Store the remainder
        \STATE $q \gets q / \beta$ % Update the quotient
        \STATE $i \gets i + 1$ % Increment index
    \ENDWHILE
    
		\STATE Gli $R[n-1], R[n-2], ..., R[0]$ rimasti (quindi gli $R[i]$ letti al contrario) sono le cifre di $X$.
\end{algorithmic}
\end{algorithm}

\par\smallskip

Notiamo come l'algoritmo permette di svolgere i calcoli \textit{nella base di partenza}, a differenza della trasformazione diretta, che si svolge \textit{nella base di arrivo}.

\subsubsection{Numeri interi in complemento a due}
Per rappresentare i numeri interi si usa, fra le altre, la rappresentazione in \textbf{complemento a due}.
Tralasciamo le basi $\beta$ arbitrarie (dove sarebbe \textit{complemento a radice}), e soffermamoci sulla base due: dettagli sulle varie altre rappresentazioni sia per i naturali che per gli interi, in basi qualsiasi, verranno riportati in un secondo momento. 
...
Per portare la rappresentazione in complemento a due $X$ su $n$ bit di un intero $x$ all'intero stesso, ci si comporta come per le rappresentazioni di naturali, ma prendendo il bit più significativo dagli $n$ bit $b_{n-1}, b_{n-2}, ... , b_1, b_0$ della rappresentazione $X$ con segno negativo:
$$
